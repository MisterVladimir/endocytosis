...
def calc_gauss_weights(sigma):
    """
    Calculate a gaussian filter kernel
    (adapted from scipy.ndimage.filters.gaussian_filter1d)
    """
    sd = float(sigma)
...
class ObjectIdentifier(list):
    def __init__(self, data, filterMode="fast", filterRadiusLowpass=1,
        ...
        """
        Creates an Identifier object to be used for object finding, takes a 2D
        or 3D slice into a data stack (data), and a filtering mode (filterMode,
        one of ["fast", "good"]) where "fast" performs a z-projection and then
        filters, wheras "good" filters in 3D before  projecting. The parameters
        filterRadiusLowpass and filterRadiusHighpass control the bandpass
        filter used to identify 'point-like' features. filterRadiusZ is the
        radius used for the axial smoothing filter.
        """
    ...
    def __FilterData2D(self, data):
        ...
        # a = ndimage.gaussian_filter(data.astype('f4'),
        #                             self.filterRadiusLowpass)
        # print(data.shape)
        ...
        _nd_image.correlate1d(data, weightsHighpass, 0, output, mode,
                              0, 0)
        _nd_image.correlate1d(output, weightsHighpass, 1, output, mode,
                              0, 0)
    ...
    def __FilterDataFast(self):
        ...
        if len(self.data.shape) == 2: #if already 2D, do nothing
            ...
            return sum([self.__FilterData2D(self.data[:, :, i]) 
    ...
    def __FilterDataGood(self):
        # lowpass filter to suppress noise
        a = ndimage.gaussian_filter(self.data, (self.filterRadiusLowpass,
                                                self.filterRadiusLowpass,
                                                self.filterRadiusZ))
        # lowpass filter again to find background 
        b = ndimage.gaussian_filter(a, (self.filterRadiusHighpass,
                                        self.filterRadiusHighpass, 0))
        # z-projection
        projData = (a - b).max(2)
        # filter again in 2D (to get rid of the projected components of 
        # the cone above and below widefield points)
        return self.__FilterData2D(projData)
    ...
    def FindObjects(self, thresholdFactor, numThresholdSteps="default",
                    blurRadius=1.5, mask=None, splitter=None, debounceRadius=4,
                    maskEdgeWidth=5, upperThreshFactor=0.5,
                    discardClumpRadius=0):
        """Finds point-like objects by subjecting the data to a
        band-pass filtering (as defined when creating the identifier) followed
        by z-projection and a thresholding procedure where the threshold is
        progressively decreased from a maximum value (half the maximum
        intensity in the image) to a minimum defined as
        [thresholdFactor]*the mode (most frequently occuring value, should
        correspond to the background) of the image. The number of steps can be
        given as [numThresholdSteps], with defualt being 5 when
        filterMode="fast" and 10 for filterMode="good". At each step the
        thresholded image is blurred with a Gaussian of radius [blurRadius] to
        approximate the image of the points found in that step, and subtracted
        from the original, thus removing the objects from the image such that
        they are not detected at the lower thresholds. This allows the
        detection of objects which are relatively close together and spread
        over a large range of intenstities. A binary mask [mask] may be
        applied to the image to specify a region (e.g. a cell) in which
        objects are to be detected. A copy of the filtered image is saved
        such that subsequent calls to FindObjects with, e.g., a different
        thresholdFactor are faster.
        """
        ...
        if self.numThresholdSteps > 0: 
            ...
            # find bin with maximum number of counts
            posMax = N.argmax()
            # bins contains left-edges - find middle of most frequent bin
            modeApp = bins[posMax:(posMax+1)].mean()

            # catch the corner case where the mode could be zero - this is
            # highly unlikely, but if it were to occur one would no longer
            # be able to influence the threshold with threshFactor
            if (abs(modeApp) < 1): 
                modeApp = 1
            ...
            if self.estSN:
                self.lowerThreshold = self.thresholdFactor*np.sqrt(
                    np.median(self.data.ravel()))
            else:
                self.lowerThreshold = self.thresholdFactor
        X, Y = np.mgrid[0:maskedFilteredData.shape[0],
        ...
        if (self.numThresholdSteps == 0):
            ...
            # do threshold scan (default)
            # generate threshold range - note slightly awkard specification of
            # lowwer and upper bounds as the stop bound is excluded from arange
            self.thresholdRange = \
                np.arange(self.upperThreshold, self.lowerThreshold -
            ...
            # use for quickly deterimining the number of pixels in a slice
            ...
            for threshold in self.thresholdRange:
                ...
                for i in range(1, nLabeled):
                    ...
                    # now work out weights for correction image
                    # (N.B. this is somewhat emperical)
                    corrWeights[objSlices[i]] = 1.0/np.sqrt(nPixels)

                # calculate correction matrix
                corr = ndimage.gaussian_filter(
                    2*self.blurRadius*np.sqrt(2*np.pi)*1.7*im*corrWeights,
                    self.blurRadius)

                # subtract from working image
                im -= corr.astype('float32')
